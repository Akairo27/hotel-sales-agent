...
الدورية `hotel-worker` (مؤقّت systemd، تمريرة كل دقيقة). **واتساب غير موصول
بالوكيل بعد**، ولهذا لا يوجد nginx ولا نفق يصل إليه: nginx يمرّر إلى اللوحة
وحدها، والوكيل مربوط بـ`127.0.0.1` فقط. **مسار دخول واتساب إلى الوكيل لم
يُحسم بعد.** اللوحة تنادي Supabase مباشرة عبر RPC، أما الوكيل والـ worker
فيتصلان بقاعدة البيانات مباشرة عبر `DATABASE_URL`. قاعدة البيانات هي مشروع
Supabase المسمّى `hotel-sales-agent-dev`.
...
127.0.0.1:8000  ◄── hotel-agent (uvicorn services.agent.main:app)
                    لا يصله شيء من الإنترنت اليوم؛ واتساب غير موصول به بعد
...
`127.0.0.1:8000`: أي مسار يوصل واتساب إليه يُصمَّم صراحةً ولا يُفتح بتغيير
عنوان الربط وحده.
...
- **لا مسار لدخول واتساب إلى الوكيل بعد.** لم يُحسم بين نفق أو وكيل عكسي أو
  غيرهما؛ والوكيل مربوط بـ`127.0.0.1:8000` فقط.
