# دليل نشر النظام — خادم التجربة

هذا الخادم (`149.104.71.71`) **نشر تجريبي (trial/staging) للنظام الحقيقي —
لا عرض توضيحي، ولا نشر إنتاجي بعد.** لا عملاء حقيقيون حتى الآن. يستضيف ثلاث
خدمات: لوحة الإدارة `hotel-admin` (Next.js على `127.0.0.1:3000` خلف nginx)،
ووكيل واتساب `hotel-agent` (FastAPI على `127.0.0.1:8000`)، ومهمة الـ worker
الدورية `hotel-worker` (مؤقّت systemd، تمريرة كل دقيقة). **واتساب موصول
بالوكيل:** nginx يمرّر المسار `/webhook/` وحده إلى `127.0.0.1:8000`، وكل ما
عداه يذهب إلى اللوحة؛ وقد تحقّق Meta من الـ webhook وتصل رسائل واتساب فعلاً
عبره. الوكيل نفسه مربوط بـ`127.0.0.1` فقط. اللوحة تنادي Supabase مباشرة عبر
RPC، أما الوكيل والـ worker فيتصلان بقاعدة البيانات مباشرة عبر `DATABASE_URL`،
كلٌّ بدوره المخصص (`hotel_agent` و`hotel_worker`، انظر «الوكيل والـ worker»
أدناه). قاعدة البيانات هي مشروع Supabase المسمّى `hotel-sales-agent-dev`.
...
الإنترنت ──443──► nginx ──proxy_pass──► 127.0.0.1:3000
            80 ──┤   │                   next start (systemd)
                 │   └─ /webhook/ ────► 127.0.0.1:8000
                 │                       hotel-agent (uvicorn services.agent.main:app)
                 └──► /var/www/certbot/.well-known/acme-challenge

hotel-worker.timer ──► hotel-worker.service   تمريرة كل دقيقة (تحرير الحجوزات المنتهية)
...
`--hostname 127.0.0.1` أو يضيف جداراً بديلاً أولاً. والأمر نفسه للوكيل على
`127.0.0.1:8000`: لا يصله من الإنترنت إلا المسار `/webhook/` عبر nginx،
ولا يُكشف مباشرة بتغيير عنوان الربط وحده. **ما يحمي `/webhook/` هو التحقق
داخل التطبيق:** توقيع `X-Hub-Signature-256` على الجسم الخام لكل `POST`
(`WHATSAPP_APP_SECRET`)، ورمز التحقق (`WHATSAPP_VERIFY_TOKEN`) لطلب `GET`.
لا `limit_req` ولا `client_max_body_size` على هذا المسار في nginx (الحد
الافتراضي `1m`)؛ انظر «قائمة ما قبل الإنتاج» في آخر هذا الملف.
...
- كتلة ٤٤٣ تمرّر أيضاً `/webhook/` إلى الوكيل على `127.0.0.1:8000`، وما عداه
  إلى اللوحة على `127.0.0.1:3000`. لا `limit_req` على `/webhook/`
...
### ٤. لا تشغّل `git` بصلاحية الجذر داخل `/srv/hotel-admin/app` إطلاقاً

المستودع مملوك لـ `www-data` بالكامل (منذ `chown -R` في المرحلة ٤). أي
`git pull` يُنفَّذ بالجذر داخل هذا المسار — ولو لمرة تشخيص عابرة — يخلّف
`.git/FETCH_HEAD` وأحياناً `.git/index` مملوكين للجذر. **الاستدعاء
التالي لـ `ops/deploy.sh`** (الذي يشغّل `git pull` بمستخدم `www-data`
عبر `su -s`، لا بالجذر) **يفشل حينها بخطأ `Permission denied`** — لأن
`www-data` لم يعد يملك كل ملفات `.git`. حدث هذا فعلياً أثناء اختبار
`ops/deploy.sh`.

**السحب اليدوي دائماً بمستخدم `www-data`، لا بالجذر مباشرة:**
```bash
su -s /bin/bash www-data -c "cd /srv/hotel-admin/app && git pull"
```

**الإصلاح عند وقوع الخطأ فعلاً** (إعادة الملكية لـ `www-data` وحدها،
بلا لمس بقية `/srv/hotel-admin/app`):
```bash
chown -R www-data:www-data /srv/hotel-admin/app/.git
```
...
- **قاعدة التجربة هي `hotel-sales-agent-dev`.** ما يُدخله العميل من اللوحة
  وما يمرّ بالوكيل يُكتب فيها. لا عملاء حقيقيون بعد، فأي تكلفة أو بيانات
  عميل حقيقية تُدخَل هنا تُدخَل في نشر تجريبي لا إنتاجي.
- **مسار واتساب `/webhook/` محميّ بالتحقق داخل التطبيق فقط.** لا حدّ معدّل ولا
  حدّ لحجم الجسم في nginx؛ البنود المؤجَّلة في «قائمة ما قبل الإنتاج» أدناه.
...
- **شاشات اللوحة الداخلية بلا أي تنسيق Tailwind — بند مفتوح.** `grep -rn
  'className=' admin/app --include='*.tsx'` يُظهر أن `layout.tsx` هو
  الملف الوحيد الذي يحوي أي className في كامل `admin/app`؛ `dashboard`،
  `login`، `hotels`، `seasons`، `price-rules`، `price-overrides`،
  `allotments`، `users` — HTML خام بلا تنسيق إطلاقاً. تحقّق هذا فعلياً
  (٢٠٢٦-٠٨-٢٤) بعد بلاغ "لا يوجد CSS" على خادم العرض — تبيّن أن البناء
  والتسليم وبناء Tailwind v4 كلها سليمة تماماً، والمُصدَّر يطابق تماماً
  ما هو موجود فعلياً في المصدر؛ المشكلة الحقيقية أن التنسيق لم يُكتب من
  الأساس، لا خلل في نطاق الاكتشاف التلقائي لـ Tailwind. مؤجَّل عمداً —
  مراحل Phase 3 (PR A–F) ركّزت على الصحة الوظيفية فقط. **تحديد نطاق
  التنسيق (كل الشاشات أم البداية بشاشة/شاشتين) يحتاج جلسة مستقلة قبل أي
  عمل.**
...
- [ ] **تدوير كلمة مرور الدور `postgres`.** تعرّض سطر `DATABASE_URL` القديم
  ...
  لا يتصل بهذا الدور الآن الوكيل ولا الـ worker (يتصلان بدورَيهما) ولا لوحة
  الإدارة (`admin.env` بلا `DATABASE_URL`). **قبل التدوير:** تحقّق من أن سر
  GitHub المسمّى `ACL_BASELINE_DATABASE_URL` (تستعمله
  `.github/workflows/acl-drift-gate.yml`) لا يشير إلى هذا المشروع؛ لا تُقرأ
...
- [ ] **حدّ معدّل على `/webhook/` في nginx.** لا `limit_req` عليه اليوم
  (على `/login` فقط)؛ الحاجز الوحيد هو التحقق من التوقيع داخل التطبيق.
- [ ] **حجم الجسم على `/webhook/`.** لا `client_max_body_size` في nginx
  (الحد الافتراضي `1m`)؛ تُراجَع كفايته لأكبر جسم webhook متوقع.
